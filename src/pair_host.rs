...
fn status(gui_sender: &UnboundedSender<GuiCommands>, message: impl Into<String>) {
    let _ = gui_sender.send(GuiCommands::PairingStatus(message.into()));
}
...
pub async fn run_pairable_host(
    ...
    match pairable_host_session(&name, &model, &mut cancel, &gui_sender).await {
        Ok(Some(pairing_file)) => {
            let _ = gui_sender.send(GuiCommands::PairingFile(Ok(PairingPayload::Remote(
                pairing_file,
            ))));
        }
        Ok(None) => {
        ...
        Err(e) => {
            let _ = gui_sender.send(GuiCommands::PairingFile(Err(e)));
        }
    }
}
